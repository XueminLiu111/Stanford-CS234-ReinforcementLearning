	...
	while True:
		...
		for state in range(nS):
			for (prob, next_state, reward, end) in P[state][policy[state]]:
				# < L2 Page22>
				new_value_function[state] += prob * (reward + value_function[next_state] * gamma)
		if np.all(np.abs(new_value_function - value_function) < tol):
			break
		value_function = new_value_function.copy()   
		# this is a deep copy
		# if change value_function, the origin new_value_function will not change
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for state in range(nS):
		...
		for action in range(nA):
			...
			# argmax(Q[action]) = new_policy   < L2 Page61 >
			if temp < Q[action]:
				temp = Q[action]
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	# iterate untill converge to the optimal policy
	...
	while True:
		...
		for state in range(nS):
			for action in range(nA):
				temp=0
				for (prob, next_state, reward, end) in P[state][action]:
					temp += prob * (reward +gamma * value_function[next_state])

				if temp > new_value_function[state]:
					new_value_function[state] = temp
